...
The command will then create a project root directory, initialize git, initialize Poetry for dependency management, and create [PEP8](https://peps.python.org/pep-0008/)-conforming a Python script template. This project is very much inspired by Rust's `cargo` package manager, which imposes opinionated formatting standards and idioms. This imposition can be tedious to comply with at first, but in the long run, it makes collaboration on Rust projects much easier. This project aims to do the same thing for a variety of languages that are common (are may become common) in scientific computing, including Python, R, Julia, Mojo, Go, Rust, and Perl.
